...
create table Course (course_id int primary key not null, course_name varchar(50) not null, description varchar (300), objectives varchar(200) not null, course_hours int not null, course_date date not null, course_start_period date not null, course_end_period date not null, total_places int not null, available_places int not null, course_fee int not null, place varchar(50) not null, check(course_start_period <= course_end_period), check (course_end_period < course_date));
